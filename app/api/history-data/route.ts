...
export async function GET(request: Request) {
    ...
    const data = await getHistoryData(user.id, queryParams.data.timeframe,{ year: queryParams.data.year, month: queryParams.data.month});
...
async function getMonthHistoryData(userId: string, year: number, month: number) {
    ...
    const history: HistoryData[] = [];
    const daysIsMonth = getDaysInMonth(new Date(year, month))
