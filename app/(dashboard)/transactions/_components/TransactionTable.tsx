...
const columns: ColumnDef<TransactionHistoryRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className={cn("capitalize rounded-lg text-center p-2", row.original.type === 'income' ? 'bg-emerald-400/10 text-emerald-500' : 'bg-red-400/10 text-red-500')}>
        {row.original.type}
      </div>
    )
  },
  {
    ...
    cell: ({ row }) => (
      <p className='text-sm rounded-lg bg-gray-400/5 p-2 text-center font-medium'>{row.original.formattedAmount}</p>
    )
  },
  {
    id: "actions", 
