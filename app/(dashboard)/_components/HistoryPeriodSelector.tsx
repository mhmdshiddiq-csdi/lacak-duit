"use client"
import { GetHistoryPeriodType } from '@/app/api/history-period/route';
...
function HistoryPeriodSelector({period, setPeriod, timeframe, setTimeframe}: Props) {
    const historyPeriod = useQuery<GetHistoryPeriodType>({
        queryKey: ["overview", "history", "periods"],
        queryFn: () => fetch(`/api/history-period`).then((res) => res.json()),
    })

    console.log(historyPeriod.data);
  return (
...
function YearSelector({period, setPeriod, years}: {period: Period, setPeriod: (period: Period) => void, years: GetHistoryPeriodType}) {
    return (
